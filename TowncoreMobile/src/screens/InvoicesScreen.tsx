      ...
      >
        ...
        <View style={styles.cardFooter}>
          {!isFreelancerInvoice && item.project?.client && (
            <Text style={styles.meta}>{item.project.client.name}</Text>
          )}
        ...
        {!isFreelancerInvoice && (
          <View style={styles.progressBar}>
            <View
              style={[
                styles.progressFill,
                { width: `${Math.min(100, (parseFloat(item.paid_amount) / parseFloat(item.total_amount)) * 100)}%` },
              ]}
            />
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  amount: { fontSize: fontSize.lg, fontWeight: '800', color: colors.primary, marginTop: 2 },
