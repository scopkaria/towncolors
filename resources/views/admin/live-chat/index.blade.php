    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-5 sm:grid-cols-3">
            ...
            @foreach ([
                ...
                ['label' => 'Closed',  'value' => $closed,  'colour' => 'text-stone-400',   'bg' => 'bg-warm-200/50 dark:bg-white/[0.04]'],
            ] as $stat)
                <div class="card-premium rounded-3xl border border-white/70 bg-white/90 p-6 shadow-card dark:border-white/[0.08] dark:bg-[#1B2632]">
        ...
        <div class="rounded-3xl border border-white/70 bg-white/90 shadow-panel dark:border-white/[0.08] dark:bg-[#1B2632]">
            @if ($sessions->isEmpty())
                <div class="px-8 py-16 text-center">
                    <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-warm-200/50 dark:bg-white/[0.04]">
            ...
            @else
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="border-b border-warm-300/40 dark:border-white/[0.06]">
                        ...
                        <tbody class="divide-y divide-warm-200/50 dark:divide-white/[0.04]">
                            @foreach ($sessions as $session)
                                <tr class="transition hover:bg-warm-200/60 dark:hover:bg-white/[0.02]">
                                    ...
                                    <td class="px-6 py-4">
                                        @php
                                            $statusColors = [
                                                ...
                                                'closed'  => 'bg-warm-200 text-stone-500 dark:bg-white/[0.06] dark:text-[#A1A1AA]',
                                            ];
                                        @endphp
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="{{ route('admin.liveChat.show', $session) }}"
                                           class="inline-flex items-center gap-1.5 rounded-xl border border-warm-300/50 bg-warm-100 px-3 py-1.5 text-xs font-semibold text-brand-ink transition hover:border-accent hover:text-accent-hover
                ...
                @if ($sessions->hasPages())
                    <div class="border-t border-warm-300/40 px-6 py-4 dark:border-white/[0.06]">
                        {{ $sessions->links() }}
                    </div>
                @endif
            @endif
