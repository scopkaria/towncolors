<x-app-layout>
    ...
    <div>
        ...
        @if ($pending->isEmpty())
            <div class="mt-4 rounded-3xl border border-dashed border-stone-200 bg-white/60 py-12 text-center">
        ...
        @else
            <div class="mt-4 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                @foreach ($pending as $item)
                    <div class="flex flex-col overflow-hidden rounded-3xl border border-amber-100 bg-white shadow-card">
                        <div class="relative h-52 overflow-hidden bg-gradient-to-br from-amber-50 to-orange-50">
                            @if ($item->image_path)
                            ...
                            @else
                                <div class="flex h-full items-center justify-center">
                                    <svg class="h-14 w-14 text-orange-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
    ...
    <div>
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ($approved as $item)
                <div class="group overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-card">
                    ...
                    <div class="p-3">
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-emerald-600">{{ $item->freelancer->name }}</p>
                        <p class="mt-0.5 text-sm font-semibold text-brand-ink line-clamp-1">{{ $item->title }}</p>
                        <form method="POST" action="{{ route('admin.portfolio.reject', $item) }}" class="mt-2">
                            @csrf @method('PATCH')
    ...
    <div>
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ($rejected as $item)
                <div class="overflow-hidden rounded-2xl border border-red-100 bg-white opacity-70 shadow-card">
                    ...
                    <div class="p-3">
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-red-400">{{ $item->freelancer->name }}</p>
                        <p class="mt-0.5 text-sm font-semibold text-brand-ink line-clamp-1">{{ $item->title }}</p>
                        <form method="POST" action="{{ route('admin.portfolio.approve', $item) }}" class="mt-2">
                            @csrf @method('PATCH')
