        ...
        <div class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-panel"
            ...
            <form method="POST" action="{{ route('admin.shop.store') }}" enctype="multipart/form-data" class="mt-5 space-y-4">
                ...
                <div x-show="mediaModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-navy-900/60 p-4 backdrop-blur-sm" @keydown.escape.window="closeMediaModal()">
                    <div class="w-full max-w-5xl rounded-3xl border border-white/70 bg-warm-100 shadow-panel" @click.outside="closeMediaModal()">
                        <div class="flex items-center justify-between border-b border-warm-300/40 px-6 py-4">
                            <div>
                                ...
                                <p class="text-xs text-brand-muted">Search and pick media just like the Media Library.</p>
                            </div>
                        ...
                        <div class="border-b border-warm-300/40 px-6 py-4">
                            <input type="text" x-model="mediaQuery" placeholder="Search media files..." class="w-full rounded-2xl border-warm-300/50 bg-warm-100 px-4 py-2.5 text-sm text-brand-ink focus:border-brand-primary focus:ring-brand-primary">
                        </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5" x-show="filteredMedia.length">
                                <template x-for="item in filteredMedia" :key="`media-${item.id}`">
                                    <button
                                        ...
                                        :class="(mediaModalMode === 'single' && String(featuredId) === String(item.id)) || (mediaModalMode === 'multi' && isGallerySelected(item.id)) ? 'border-brand-primary ring-2 ring-brand-primary/30' : 'border-warm-300/50 hover:border-brand-primary/40'"
                                        @click="mediaModalMode === 'single' ? pickFeatured(item.id) : toggleGallery(item.id)">
                        ...
                        <div class="flex items-center justify-between border-t border-warm-300/40 px-6 py-4">
                            ...
                            <button type="button" class="btn-primary" @click="closeMediaModal()">Done</button>
                        </div>
                    </div>
                </div>
