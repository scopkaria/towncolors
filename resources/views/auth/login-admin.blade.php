...
<head>
    ...
    <title>Admin Sign In — Towncore</title>
        ...
        <aside class="relative hidden lg:flex lg:w-[46%] xl:w-[44%] flex-col justify-between overflow-hidden px-12 py-12"
            ...
            <div class="relative z-10 space-y-6 my-auto py-14">
                ...
                <p class="text-[0.9rem] leading-7 text-stone-400 max-w-sm">
                    Full platform control — users, projects, analytics, billing, and system configuration. Authorised access only.
                </p>
        ...
        <main class="flex flex-1 flex-col items-center justify-center overflow-y-auto px-5 py-12 sm:px-8 lg:px-14 xl:px-20"
            ...
            <div class="w-full max-w-md animate-[page-fade_0.55s_ease-out_both]">
                ...
                <div class="mb-8">
                    ...
                    <h2 class="font-display text-3xl font-bold text-white mb-1.5">Admin sign in</h2>
                    <p class="text-sm text-stone-500">Authorised personnel only.</p>
                </div>
                ...
                {{-- No external links — admin URL is private and sent directly by platform owner --}}
                <p class="mt-8 text-center text-xs text-stone-700">
    ...
    <div class="w-full h-full lg:min-h-screen flex flex-col lg:flex-row max-w-[1200px] mx-auto lg:max-w-none lg:fixed lg:inset-0">
        ...
        <aside class="relative hidden lg:flex lg:w-[46%] xl:w-[42%] flex-col justify-between overflow-hidden bg-[#07090f] px-12 py-12">
            ...
            <div class="relative z-10 space-y-7 my-auto py-16">
                ...
                <h1 class="font-display text-4xl xl:text-5xl font-bold leading-tight text-white">
                    Platform<br>
                    <span class="bg-gradient-to-r from-amber-300 to-amber-500 bg-clip-text text-transparent">command centre.</span>
                </h1>
                <p class="text-base leading-8 text-slate-400 max-w-sm">
                    Administrate the full platform — users, projects, transactions, system settings, and real-time analytics.
                </p>
                ...
                <div class="space-y-3 pt-2">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-amber-500/15">
                            <svg class="h-3 w-3 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                        </div>
                ...
                <div class="rounded-xl border border-amber-500/15 bg-amber-500/5 p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <p class="text-xs leading-5 text-amber-200/70">
                            This portal is for authorised platform administrators only. All sign-in attempts are logged.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <main class="flex flex-1 flex-col items-center justify-center overflow-y-auto bg-[#080c12] px-5 py-12 sm:px-8 lg:px-14 xl:px-20">
            ...
            <div class="w-full max-w-md animate-[page-fade_0.55s_ease-out_both]">
                ...
                {{-- Status flash --}}
                @if (session('status'))
                    <div class="mb-6 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <div class="mb-8 space-y-2">
                    ...
                    <h2 class="font-display text-3xl font-bold text-white">Admin sign in</h2>
                    <p class="text-sm text-slate-400 leading-6">Authorised personnel only. Sign-in activity is monitored.</p>
                </div>
                ...
                <form method="POST" action="{{ route('login.admin.post') }}" class="space-y-5" novalidate>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-amber-400 hover:text-amber-300 transition-colors">Forgot password?</a>
                            @endif
                        </div>
                        <input id="password" name="password" type="password"
                               required autocomplete="current-password"
                    ...
                    <div class="flex items-center gap-3">
                        <input id="remember_me" name="remember" type="checkbox"
                               class="h-4 w-4 rounded border-slate-800 bg-slate-900 text-amber-500 focus:ring-amber-500/20 focus:ring-offset-[#080c12]" />
                ...
                <div class="mt-8 border-t border-slate-800/60 pt-6 text-center">
                    <p class="text-sm text-slate-500">
                        Need admin access?
                        <a href="{{ route('register.admin') }}" class="font-semibold text-amber-400 hover:text-amber-300 transition-colors">Request it here</a>
                    </p>
                </div>
            </div>
        </main>
