<x-app-layout>
    ...
    <div x-data="globalChat()"
                    ...
                    <template x-for="group in contactGroups" :key="group.name">
                        <div>
                            ...
                            <template x-for="u in group.users" :key="u.id">
                                <div class="flex items-center gap-3 px-3 py-2.5 transition hover:bg-stone-50">
                                    ...
                                    <button @click="startDirectChat(u.id)" title="Send message"
                                            class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-stone-200 text-brand-muted transition hover:border-orange-300 hover:bg-orange-50 hover:text-brand-primary">
        ...
        <div class="flex min-w-0 flex-1 flex-col"
            ...
            <div x-show="!activeConversation" x-cloak
                ...
                <div class="rounded-3xl bg-gradient-to-br from-orange-50 to-amber-50 p-8">
            ...
            <div x-show="activeConversation" x-cloak class="flex min-h-0 flex-1 flex-col">
                ...
                <div class="flex shrink-0 items-center gap-3 border-b border-stone-100 px-5 py-4 dark:border-white/[0.06]">
                    ...
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl font-display text-sm font-bold"
                         :class="activeConversation?.avatar_color === 'orange' ? 'bg-orange-100 text-brand-primary' : 'bg-blue-100 text-blue-600'">
                    ...
                    <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider"
                          :class="activeConversation?.type === 'project' ? 'bg-orange-50 text-brand-primary' : 'bg-blue-50 text-blue-600'"
                          x-text="activeConversation?.type === 'project' ? 'Project' : 'Direct'"></span>

                    {{-- Voice call --}}
                    <button @click="openCall(activeConversation.room_name, 'audio')"
                            title="Voice call"
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-stone-200 text-brand-muted transition hover:border-green-300 hover:bg-green-50 hover:text-green-600">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    ...
                    <button @click="openCall(activeConversation.room_name, 'video')"
                            title="Video call"
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-stone-200 text-brand-muted transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        ...
                                        <template x-if="msg.message_type === 'audio' && msg.file_path">
                                            <div>
                                                <div class="flex items-center gap-3 px-3 py-2.5">
                                                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full" :class="msg.sender_id == currentUserId ? 'bg-white/20' : 'bg-orange-50'">
                                        ...
                                        <template x-if="msg.message_type === 'location' && msg.latitude">
                                            <div>
                                                <a :href="'https://www.google.com/maps?q=' + msg.latitude + ',' + msg.longitude" target="_blank" rel="noopener" class="block">
                                                    <div class="flex h-24 items-center justify-center bg-stone-100">
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <div class="relative">
                            ...
                            <div x-show="showEmoji" x-cloak @click.outside="showEmoji = false"
                                 ...
                                 class="absolute bottom-12 left-0 z-50 max-h-72 w-72 overflow-y-auto rounded-2xl border border-stone-200 bg-white p-3 shadow-panel">
                                <div class="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-brand-muted">Smileys</div>
                                <div class="grid grid-cols-8 gap-0.5">
                                    <template x-for="emoji in emojis" :key="emoji">
                                        <button type="button" @click="insertEmoji(emoji)" class="flex h-8 w-8 items-center justify-center rounded-lg text-lg transition hover:scale-110 hover:bg-orange-50" x-text="emoji"></button>
                                    </template>
                                </div>
                        ...
                        <div class="relative">
                            ...
                            <div x-show="showAttach" x-cloak @click.outside="showAttach = false"
                                ...
                                <button type="button" @click="$refs.cameraInput.click(); showAttach=false" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-50">
                                ...
                                <button type="button" @click="$refs.photoInput.click(); showAttach=false" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-50">
                                ...
                                <button type="button" @click="$refs.docInput.click(); showAttach=false" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-50">
                                ...
                                <button type="button" @click="sendLocation(); showAttach=false" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-50">
                                ...
                                <button type="button" @click="startRecording(); showAttach=false" :disabled="isRecording" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-stone-50 disabled:opacity-50">
                                    <div class="flex h-9 w-9 items-center justify-center rounded-full bg-orange-100"><svg class="h-4 w-4 text-brand-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z"/></svg></div>
            ...
            <div class="p-4">
                <div class="relative mb-3">
                    ...
                    <input type="text" x-model="userSearch" placeholder="Search people…"
                           class="w-full rounded-2xl border-0 bg-stone-100 py-2 pl-9 pr-4 text-sm focus:bg-white focus:ring-2 focus:ring-orange-200">
                </div>
                <div class="max-h-64 space-y-1 overflow-y-auto">
                    ...
                    <template x-for="u in filteredUsers" :key="u.id">
                        <button @click="startDirectChat(u.id)"
                                class="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition hover:bg-orange-50">
    ...
    <div x-show="showCallModal" x-cloak
        ...
        <div x-ref="jitsiContainer"
             id="jitsi-container"
             class="min-h-0 flex-1 w-full bg-slate-950">
        </div>
    </div>
