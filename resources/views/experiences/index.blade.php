...
{{-- ╔══════════════════════════════════════════════════════════════════╗
     ║  HERO                                                           ║
     ╚══════════════════════════════════════════════════════════════════╝ --}}
<section class="relative overflow-hidden bg-white border-b border-stone-100 py-16 sm:py-24">
            ...
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($experiences as $index => $experience)
                    <a href="{{ route('services.show', $experience) }}"
                       class="reveal reveal-delay-{{ ($index % 3) + 1 }} group relative flex flex-col overflow-hidden rounded-3xl border border-stone-100 bg-white shadow-card transition duration-300 ease-out hover:-translate-y-1 hover:border-brand-primary/25 hover:shadow-[0_28px_55px_-20px_rgba(249,115,22,0.18)]">
                        ...
                        <div class="relative h-52 shrink-0 overflow-hidden sm:h-56">
                            ...
                            @else
                                ...
                                <div class="flex h-full w-full items-center justify-center"
                                     style="background: linear-gradient(135deg, {{ $experience->color ?? '#f97316' }}22 0%, {{ $experience->color ?? '#f97316' }}0a 100%);">
                                    <svg class="h-16 w-16 opacity-20" fill="none" viewBox="0 0 24 24"
                                         stroke="{{ $experience->color ?? '#f97316' }}" stroke-width="1">
                        ...
                        <div class="flex flex-1 flex-col gap-3 p-6">
                            ...
                            <div class="flex items-start gap-3">
                                <span class="mt-1 h-3 w-3 shrink-0 rounded-full"
                                      style="background-color: {{ $experience->color ?? '#f97316' }};"></span>
                            ...
                            <div class="mt-auto flex flex-wrap items-center gap-3 pt-2">
                                @if ($experience->price_range)
                                    <span class="inline-flex items-center gap-1 rounded-full border border-stone-200 bg-stone-50 px-3 py-1 text-xs font-semibold text-brand-muted">
