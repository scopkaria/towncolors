    ...
    @else
        <div class="space-y-3">
            @foreach ($projects as $project)
                <article
                    class="group rounded-3xl border border-white/70 bg-white/90 p-5 shadow-card transition duration-200 hover:border-accent/30 hover:shadow-panel"
                    role="link"
                    tabindex="0"
                    onclick="window.location='{{ route('freelancer.projects.show', $project) }}'"
                    onkeydown="if(event.key==='Enter' || event.key===' '){ event.preventDefault(); window.location='{{ route('freelancer.projects.show', $project) }}'; }"
                >
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-3">
                                <h3 class="font-display text-2xl text-brand-ink transition duration-200 group-hover:text-brand-primary">{{ $project->title }}</h3>
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-4 text-xs text-brand-muted">
                                <span>{{ $project->created_at->diffForHumans() }}</span>
                                <span class="inline-flex h-1 w-1 rounded-full bg-warm-400"></span>
                                <span class="font-medium text-brand-ink">{{ $project->client->name }}</span>
                            </div>
                        </div>

                        <div class="shrink-0">
                            <a
                                href="{{ route('chat.show', $project) }}"
                                class="inline-flex items-center gap-2 rounded-2xl border border-accent/30 bg-accent-light/60 px-4 py-2.5 text-xs font-semibold text-brand-primary transition hover:bg-brand-primary hover:text-white"
